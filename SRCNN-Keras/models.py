...
try:
    import cv2
    _cv2_available = True
    warnings.warn('CV2 IS AVAILABLE.')
except:
...
class BaseSuperResolutionModel(object):
    ...
    def fit(self, batch_size=128, nb_epochs=100, save_history=True, history_fn="Model History.txt") -> Model:
        ...
        """
        samples_per_epoch = img_utils.image_count() # Train image count
        val_count = img_utils.val_image_count() # Validation image count
        ...
        self.model.fit_generator(
            img_utils.image_generator(train_path, scale_factor=self.scale_factor, small_train_images=self.type_true_upscaling, batch_size=batch_size),
                                 steps_per_epoch=samples_per_epoch // batch_size + 1,
                                 epochs=nb_epochs, callbacks=callback_list,
                                 validation_data=img_utils.image_generator(validation_path, scale_factor=self.scale_factor, small_train_images=self.type_true_upscaling, batch_size=batch_size),
    ...
    def upscale(self, img_path, save_intermediate=False, return_image=False, suffix="scaled",
        ...
        true_img = io.imread(img_path) #, mode='RGB')
        logger.info("Read image with width {}", (len(true_img[0])))
        
        init_dim_1, init_dim_2 = true_img.shape[0], true_img.shape[1]
        if verbose: logger.info("Old Size : ", true_img.shape)
        if verbose: logger.info("New Size : (%d, %d, 3)" % (init_dim_1 * scale_factor, init_dim_2 * scale_factor))
        ...
        if mode == "patch" and self.type_true_upscaling:
            ...
            mode = 'fast'
            logger.error("Patch mode does not work with True Upscaling models yet. Defaulting to mode='fast'")

        if mode == 'patch':
            # Create patches
            if self.type_requires_divisible_shape:
                if patch_size % 4 != 0:
                    logger.error("Deep Denoise requires patch size which is multiple of 4.\nSetting patch_size = 8.")
                    patch_size = 8
            ...
            logger.info("Number of patches = {}, Patch Shape = ({}, {})" % (nb_images, img_dim_2, img_dim_1))
        else:
        ...
        print("\nCalling create model xxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
...
def _evaluate(sr_model : BaseSuperResolutionModel, validation_dir, scale_pred=False):
    ...
    for val_dir in validation_dirs:
        ...
        for impath in os.listdir(val_dir):
            ...
            # Input image
            y = io.imread(val_dir + impath, mode='RGB')
            ...
            if sr_model.type_requires_divisible_shape:
                ...
                if ((width // sr_model.scale_factor) % 4 != 0) or ((height // sr_model.scale_factor) % 4 != 0) \
                    ...
                    y = resize(y, (width, height), interp='bicubic')
            ...
            img = resize(x_temp, (x_width, x_height),
                                     interp='bicubic')

            if not sr_model.type_true_upscaling:
                img = resize(img, (x_width, x_height), interp='bicubic')
            ...
            io.imsave(generated_path, y_pred)
...
def _evaluate_denoise(sr_model : BaseSuperResolutionModel, validation_dir, scale_pred=False):
    ...
    for val_dir in validation_dirs:
        ...
        for impath in os.listdir(val_dir):
            ...
            img = resize(x_temp[0], (width // sr_model.scale_factor, height // sr_model.scale_factor),
                                     interp='bicubic', mode='RGB')

            if not sr_model.type_true_upscaling:
                img = resize(img, (width, height), interp='bicubic')
...
class ImageSuperResolutionModel(BaseSuperResolutionModel):
    ...
    def __init__(self, scale_factor):
        ...
        #print("kpl - weight path : " + weight_path)
    ...
    def create_model(self, height=32, width=32, channels=3, load_weights=False, batch_size=128):
        ...
        print("Create model called: RUNNING 3 Convolutions (9x9), (1x1), (5x5) using 'relu', then Adam optimizer     - kpl")
        ...
        print("Compiling ...........with load weigths = " + str(load_weights))
...
class ExpantionSuperResolution(BaseSuperResolutionModel):
    ...
    def create_model(self, height=32, width=32, channels=3, load_weights=False, batch_size=128):
        """
            Creates a model to be used to scale images of specific height and width.
        """
        print("Create model called: RUNNING 4 Convolutions using 'relu', and average, another conv., then Adam optimizer     - kpl")
        ...
        x = Average()([x1, x2, x3])
        print("Model created - average - kpl")
        ...
        self.model = model
        print("Model created - returning - kpl")
        return model
