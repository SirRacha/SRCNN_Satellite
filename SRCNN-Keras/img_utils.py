...
# Train images go in your HOME / train2014
input_path = os.path.expanduser('~') + "/train2014/" 
...
def transform_images(directory, output_directory, scaling_factor=2, max_nb_images=-1, true_upscale=False):
    ...
    logger.info("Preparing to save data to %s and %s".format(output_directory + "X/", output_directory + "y/"))
    ...
    for file in os.listdir(directory):
        ...
        logger.info("hr patch size is %d %d " % (scaling_factor, _image_scale_multiplier))
        nb_hr_images = (img_size ** 2) // (stride ** 2) # e.g. 256 ** 2 / 64 ** 2
        ...
        for i in range(nb_hr_images):
            ...
            output_path_temp = output_directory + "/y/" + "%d_%d.png" % (index, i + 1)
            logger.info("Saving {}".format(output_path_temp))
            io.imsave(output_path_temp, ip)

            # Apply Gaussian Blur to Y
            op = gaussian_filter(ip, sigma=0.5)

            # Subsample by scaling factor to Y
            op = resize(op, (lr_patch_size, lr_patch_size))

            if not true_upscale:
                # Upscale by scaling factor to Y
                op = resize(op, (hr_patch_size, hr_patch_size))

            # Save Y
            io.imsave(output_directory + "/X/" + "%d_%d.png" % (index, i+1), op)
...
def transform_images_temp(directory, output_directory, scaling_factor=2, max_nb_images=-1, true_upscale=False,
    ...
    for file in os.listdir(directory):
        ...
        hr_patch_size = 64
        lr_patch_size = 32
        nb_hr_images = (img_size ** 2) // (stride ** 2) # e.g. = 4

        hr_samples = np.empty((nb_hr_images, hr_patch_size, hr_patch_size, 3)) # e.g. 4, 64, 64, 3
        ...
        for i in range(nb_hr_images):
            ip = hr_samples[i]
            # Save ground truth image X
            io.imsave(output_directory + "/y/" + "%d_%d.png" % (index + id_advance, i + 1), ip)

            # Apply Gaussian Blur to Y
            #op = gaussian_filter(ip, sigma=0.5)

            # Subsample by scaling factor to Y
            op = resize(ip, (lr_patch_size, lr_patch_size)) # TODO -KPL, interp='bicubic')
            ...
            # Save Y
            io.imsave(output_directory + "/X/" + "%d_%d.png" % (index + id_advance, id_advance + i + 1), op)
...
def image_generator(directory, scale_factor=2, target_shape=None, channels=3, small_train_images=False, shuffle=True,
        ...
        if small_train_images:
            ...
            if K.image_dim_ordering() == "th":
                ...
                image_shape = (32 * scale_factor * _image_scale_multiplier, 32 * scale_factor * _image_scale_multiplier,
                               channels)
                # TEST kpl image_shape=(32, 32, 3)
                y_image_shape = image_shape
                
                print("Images shape " + str(y_image_shape))
    else:
        if small_train_images:
            if K.image_dim_ordering() == "th":
    ...
    while 1:
        ...
        if count_used_for_log % 50 == 0:
            logger.info("Shapes are %s and %s" % (batch_x.shape, batch_y.shape))

        try:
            for i, j in enumerate(index_array):
                x_fn = X_filenames[j]
                img = io.imread(x_fn)
                if count_used_for_log % 100 == 0:
                    logger.debug("Read file {} with shape {}".format(x_fn, img.shape))                
                if small_train_images:
                ...
                if K.image_dim_ordering() == "th":
                    ...
                    batch_x[i] = img # kpl - problem is here
    
                y_fn = y_filenames[j]
                if count_used_for_log % 100 == 0:
                    logger.debug("Read file {} ".format(y_fn))
                img = io.imread(y_fn)
                img = img.astype('float32') / 255.
    
                logger.info("Image that was read has size {}".format(img.shape))
...
if __name__ == "__main__":
    ...
    true_upscale = False # Set to False for training new model - kpl

    # TODO - make configurable
    # To create the training and validation folders, the next to lines need to be ran 1 at a time
#    transform_images(input_path, output_path, scaling_factor=scaling_factor, max_nb_images=max_images_to_run, true_upscale=true_upscale)
    transform_images(validation_set5_path, validation_output_path, scaling_factor=scaling_factor, max_nb_images=max_images_to_run, true_upscale=true_upscale)
